...
export async function openTrayDialog(app, { pick = null, frame = null, title = null } = {}) {
  ...
  const dlg = modal({ title: title || 'Use a sim from my Tray',
    text: pick ? 'Adults only. Your animation doesn\'t change - you only see it on their body for a while.' : 'Adults only. The sim comes in with their body shape, skin and hair (clothes come later).',
