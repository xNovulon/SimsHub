...
//   node tools/e2e.js --port 8842 [--only 1,2,3] [--out <folder>]
// The server must already run on that port (start one with ANIMATOR_PORT=<port> python backend\server.py; the user's
// app on 8765, the Sims Hub on 8766 and the verifier on 8777 are refused).
// Nothing is written anywhere by the app: the test harness (tools/checks/lib/harness.js, memory mode) answers every
// writing request in the browser and keeps saved animations, progressions and My poses in the test's memory. The
// packages Send to game and Export a mod would write are built offline into %TEMP% (tools/checks/lib/offline_pkg.py)
// and checked there (cache/e2e/verify_pkg.py). Screenshots and report.json end up in cache/e2e/.
const path = require('path'), fs = require('fs'), os = require('os'), cp = require('child_process');
const H_ = require('./checks/lib/harness.js');

const ROOT = path.join(__dirname, '..');
const argv = process.argv.slice(2);
const argOf = (k, d) => { const i = argv.indexOf(k); return i >= 0 && argv[i + 1] ? argv[i + 1] : d; };
const PORT = String(argOf('--port', process.env.ANIMATOR_PORT || ''));
if (!/^\d+$/.test(PORT) || H_.REFUSED.has(+PORT)) {
  console.error('usage: node tools/e2e.js --port <test server port>   (not 8765, 8766 or 8777)');
  process.exit(2);
}
process.env.ANIMATOR_PORT = PORT;                // the package verifier reads the rig from the same server
const OUT = path.resolve(argOf('--out', process.env.E2E_OUT || path.join(ROOT, 'cache', 'e2e')));
const APP_URL = `http://127.0.0.1:${PORT}/`;
const DOCS = path.join(os.homedir(), 'Documents');
const SIMS = path.join(DOCS, 'Electronic Arts', 'The Sims 4');
const FITSTUDIO = path.join(SIMS, 'Mods', 'FitStudio');
...
const SAVES = path.join(SIMS, 'saves', 'FitStudio');
const W = 1600, H = 900;
const onlyArg = argOf('--only', /^[\d,]+$/.test(argv[0] || '') ? argv[0] : null);
const ONLY = onlyArg ? new Set(onlyArg.split(',')) : null;   // e.g. "1,2,3" runs only those steps (1 always runs)
let harness = null;                                            // {browser, page, logs, writes, passed, memory}
...
// A read-only picture of a folder (names, sizes and change times), to prove nothing was written there.
function folderState(d, depth = 1) {
  const out = {};
  for (const n of listDir(d)) {
    const p = path.join(d, n);
    let st;
    try { st = fs.statSync(p); } catch { continue; }
    if (st.isDirectory()) { if (depth > 0) for (const [k, v] of Object.entries(folderState(p, depth - 1))) out[n + '/' + k] = v; }
    else out[n] = `${st.size}:${Math.round(st.mtimeMs)}`;
  }
  return out;
}
...
// The package Send to game / Export a mod would write, built offline into %TEMP% (never into Documents).
function offline(kind, body, tag) {
  const mem = harness && harness.memory;
  const job = { kind, body, projects: mem ? [...mem.projects.values()].map(x => x.project) : [], progressions: mem ? mem.progressions : null };
  const jf = path.join(os.tmpdir(), `wa_e2e_job_${tag}_${process.pid}.json`);
  fs.writeFileSync(jf, JSON.stringify(job));
  const r = cp.spawnSync('python', [path.join(__dirname, 'checks', 'lib', 'offline_pkg.py'), jf], { encoding: 'utf8', maxBuffer: 256 << 20, env: { ...process.env } });
  try { fs.unlinkSync(jf); } catch { /* ignore */ }
  const last = (r.stdout || '').trim().split('\n').pop();
  let res = null;
  try { res = JSON.parse(last); } catch { res = { error: 'offline build printed no JSON', stderr: (r.stderr || '').slice(-1500), out: last } }
  return res;
}
// The bodies of the writing requests the page made since `from` (answered in the browser).
const writesOf = (route, from = 0) => harness.writes.slice(from).filter(w => w.route === route).map(w => { try { return JSON.parse(w.body); } catch { return null; } });

function verifyPkg(mode, pkg, expect, tag) {
  const ef = path.join(OUT, `expect_${tag}.json`);
  fs.writeFileSync(ef, JSON.stringify(expect));
  const r = cp.spawnSync('python', [path.join(ROOT, 'cache', 'e2e', 'verify_pkg.py'), mode, pkg, ef], { encoding: 'utf8', maxBuffer: 256 << 20, env: { ...process.env, ANIMATOR_PORT: PORT } });
  const last = (r.stdout || '').trim().split('\n').pop();
  let j = null;
...
async function installHelpers() {
  await page.evaluate(async () => {
    ...
    // (a dialog that is closing - fading out - does not count)
    E.modal = title => [...document.querySelectorAll('#modal-root .backdrop:not(.leaving) .modal')].find(m => !title || (m.querySelector('h2')?.textContent || '').includes(title)) || null;
...
(async () => {
  ...
  // what is on disk before the test (read only): nothing here may change
  const before = {
    myAnim: listDir(MYANIM), soundKit: fs.existsSync(SOUNDKIT), exports: fs.existsSync(EXPORTS),
    exportsList: listDir(EXPORTS),
    state: { saves: folderState(SAVES, 1), myAnim: folderState(MYANIM, 0), exports: folderState(EXPORTS, 1) },
  };
  info.before = { myAnim: before.myAnim.length, exports: before.exportsList.length, saves: Object.keys(before.state.saves).length };

  const initialServer = await (async () => {
    const j = async u => (await fetch(APP_URL.slice(0, -1) + u)).json();
    return { projects: (await j('/api/projects')).map(p => p.file), progressions: await j('/api/progressions') };
  })();
  info.initialServer = initialServer;

  const t0 = Date.now();
  harness = await H_.open(PORT, { w: W, h: H, memory: true, timeout: 180000 });
  browser = harness.browser; page = harness.page;
  info.loadMs = Date.now() - t0;
  for (const l of harness.logs) if (['error', 'pageerror', 'requestfailed'].includes(l.type)) logs.push({ step: 'boot', ...l });
  ...
  await step(1, 'load, Home, Woman & man, name dialog', async () => {
    // the harness opened the app (with ?slot=test) and waited for its ready signal
    const loaded = await page.evaluate(() => document.getElementById('loading').classList.contains('done')).catch(() => false);
    check('app starts (#loading.done)', loaded, { ms: info.loadMs, proxy: harness.proxy });
    ...
    await shot('home');
    // "Woman & man" (a pill on the Blank animation card; older Homes had a start card of its own)
    await page.evaluate(() => {
      const pill = [...document.querySelectorAll('#home .start-pill, #home .start')].find(b => /Woman & man/.test(b.textContent) && (b.classList.contains('start-pill') || b.classList.contains('feature')));
      (pill || document.querySelector('#home .start.feature')).click();
    });
  ...
  await step(2, 'every rail step renders', async () => {
    const TITLES = { scene: 'Scene', pose: 'Pose', motion: 'Motion', body: 'Body', face: 'Face', sounds: 'Sounds & moments', details: 'Details', share: 'Share', library: 'Library' };
  ...
  await step(3, 'ready poses (couple + solo) and Pose tools', async () => {
    ...
    await pe(async () => {
      ...
      if (dlg) {
        ...
        await __m.api.saveMyPoses((await __m.api.myPoses()).filter(x => x.label !== 'E2E pose'));
  ...
  await step(4, 'motion layers', async () => {
    const d = await pe(async () => {
      ...
      const WATCH = ['b__Pelvis__', 'b__Spine2__', 'b__Head__', 'b__R_Hand__', 'b__L_Calf__', 'b__L_Eye__'];
      const who = { thrust: 'M', ride: 'F', grind: 'F', bounce: 'F', twerk: 'F', sway: 'F', headbob: 'F', stroke: 'F', breathe: 'M' };
      const mainOf = { stroke: 'b__R_Hand__', headbob: 'b__Head__', breathe: 'b__Head__', look: 'b__L_Eye__', tremble: 'b__L_Calf__' };
      const STILL = new Set(['look']);        // turns and holds: what it changes is measured with its switch
      const sample = (s, L) => {
      ...
      for (const type of Object.keys(MOTIONS)) {
        ...
        let amp = Math.max(...tr[mi].map(q => E.dist(q, tr[mi][0])));
        ...
        if (sw) {
          ...
          if (STILL.has(type)) amp = E.dist(tr[mi][0], a0);
  ...
  await step(8, 'Keys: key, key all, delete, eases, tween, undo/redo, timeline, length', async () => {
    ...
    const g = await page.evaluate(() => {
      ...
      app.timeline.fit();
      // lane geometry from the timeline itself (features may add rows above the lanes)
      const tl = app.timeline;
      return { left: r.left, top: r.top, h: r.height, w: r.width, row, x30: r.left + tl.xAt(30), x50: r.left + tl.xAt(50), x60: r.left + tl.xAt(60), x70: r.left + tl.xAt(70),
        y: r.top + tl.laneTop(row) + 15 * tl.lane / 48, lanesEnd: tl.laneTop(app.store.project.sims.length - 1) + tl.lane, ppf: tl.pxPerFrame };
    });
    info.timeline = g;
    check('timeline canvas is tall enough for both lanes', g.h >= g.lanesEnd, { h: g.h, lanesEnd: g.lanesEnd });
  ...
  await step(10, 'Send to game + package check', async () => {
    ...
    // the real button and dialog (the harness answers Send to game in the browser: nothing is written)
    const w0 = harness.writes.length;
    await page.click('#btn-export');
    await page.waitForFunction(() => __e2e.modal('Send to game'), { timeout: 8000 });
    await pe(() => {
      const out = [], C = (n, ok, d) => out.push([n, !!ok, d]);
      const m = __e2e.modal('Send to game');
      C('Send to game dialog: no red items (nothing blocks sending)', !m.querySelector('.warn-box.error'), [...m.querySelectorAll('.warn-box')].map(x => x.textContent));
      const notes = [...m.querySelectorAll('.warn-box:not(.error)')].map(x => x.textContent);
      C('Send to game dialog: its notes are only ones a feature can explain (clipping has "Show me")', notes.every(t => !/go through/.test(t) || /Show me/.test(t)), notes);
    ...
    const sent = writesOf('/api/export', w0);
    check('Send to game sent the baked animation (answered in the browser - nothing written)', sent.length === 1 && sent[0] && Array.isArray(sent[0].actors) && sent[0].actors.length === 2, sent.length);
    check('the dialog saved the project before sending (answered in the browser)', writesOf('/api/project', w0).length >= 1);
    check('the success card shows where it goes', !!uiPath, uiPath);
    // the package, built offline in %TEMP% from exactly what the dialog sent
    const b = await page.evaluate(() => app.bake());
    exportedA = { b };
    fs.writeFileSync(path.join(OUT, 'bake_A.json'), JSON.stringify(b));
    check('a fresh bake equals what the dialog sent', sent[0] && JSON.stringify(sent[0]) === JSON.stringify(b));
    const res = offline('export', sent[0] || b, 'A');
    check('the package is built offline in %TEMP% (never in the Mods folder)', res && res.path && fs.existsSync(res.path) && path.resolve(res.path).toLowerCase().startsWith(path.resolve(os.tmpdir()).toLowerCase()), res && (res.path || res));
    check('export result names both clips', res && res.clips && res.clips.length === 2, res && res.clips);
    info.exportA = res;
    if (res && res.path) info.exportAPackage = verifyPkg('single', res.path, { bakes: [sent[0] || b] }, 'single');
    check('the Mods folder was not touched', listDir(MYANIM).sort().join() === before.myAnim.slice().sort().join() && fs.existsSync(SOUNDKIT) === before.soundKit, listDir(MYANIM).filter(f => /E2E/.test(f)));
  });
  ...
  await step(11, 'save A and B, progression, export a mod', async () => {
    ...
    uidC = stale;
    // Export a mod through the dialog (answered in the browser; the package is built offline below)
    const wB = harness.writes.length;
    ...
    const uiReq = writesOf('/api/bundle', wB)[0];
    check('Export a mod sent the picked animations (answered in the browser - nothing written)', uiReq && Array.isArray(uiReq.animations) && uiReq.animations.length === 2, uiReq && uiReq.animations && uiReq.animations.length);
    // direct bundle
    const wD = harness.writes.length;
    const r = await page.evaluate(async (uidA, uidB) => {
      ...
      await __m.api.bundle({ name: 'E2E Test Pack', author: 'E2E', animations: bakes, include_sounds: true, install: false });
      ...
      return { bakes, packable, progs: await __m.api.progressions() };
    }, uidA, uidB);
    const directReq = writesOf('/api/bundle', wD)[0];
    fs.writeFileSync(path.join(OUT, 'bundle_bakes.json'), JSON.stringify(r.bakes));
    // the packages, built offline in %TEMP% with the saved animations and progressions the test kept in memory
    const res = directReq ? offline('bundle', directReq, 'bundle') : { error: 'no bundle request' };
    info.bundle = res;
    check('the mod is built offline in %TEMP%', res && res.package && fs.existsSync(res.package) && path.resolve(res.package).toLowerCase().startsWith(path.resolve(os.tmpdir()).toLowerCase()), res && (res.package || res.error || res));
    const prog = r.progs.find(g => g.id === progId);
    const expect = {
      bakes: r.bakes, packable: r.packable, readme: res && res.folder ? path.join(res.folder, 'README.txt') : null, zip: res && res.zip, progression: prog && prog.name,
      next: { 'E2E Test A': ['E2E Test B'], 'E2E Test B': [] }, random: { 'E2E Test A': true, 'E2E Test B': false },
    };
    check('bundle result: 2 animations, the progression, sounds packed', res && res.animations === 2 && (res.progressions || []).length === 1 && (r.packable.length === 0 || res.sounds_packed > 0),
      res && { animations: res.animations, progressions: res.progressions, sounds_packed: res.sounds_packed, packable: r.packable, missing: res.missing_sounds, error: res.error });
    if (res && res.package) info.bundlePackage = verifyPkg('bundle', res.package, expect, 'bundle');
    const uiRes = uiReq ? offline('bundle', uiReq, 'bundle_ui') : null;
    if (uiRes && uiRes.package) {
      const ex2 = { ...expect, bakes: uiReq.animations, readme: path.join(uiRes.folder, 'README.txt'), zip: uiRes.zip };
      info.bundleUiPackage = verifyPkg('bundle', uiRes.package, ex2, 'bundle-dialog');
    } else check('dialog bundle package builds offline', false, uiRes && (uiRes.error || uiRes));
    check('Documents\\Wicked Animator Exports was not touched', !listDir(EXPORTS).some(f => /^E2E/.test(f)), listDir(EXPORTS).filter(f => /E2E/.test(f)));
    check('MyAnimations was not touched', listDir(MYANIM).sort().join() === before.myAnim.slice().sort().join(), listDir(MYANIM).filter(f => /E2E/.test(f)));
  });
  ...
  await step(16, 'two animations with the same name', async () => {
    ...
    check('saving another animation under an existing name keeps the first one (both listed)', col.listed.includes(col.X) && col.listed.includes(col.Y), col);
    // the app asks first when the name is taken by another animation, and "Save as" keeps both
    const ask = await page.evaluate(async () => {
      const E = __e2e, base = JSON.parse(JSON.stringify(app.store.project));
      const Z = { ...base, uid: __m.state.uid('a'), name: 'E2E Test Collide', loops: 5 }; delete Z.thumb;
      app.store.load(Z);
      const saving = app.save();
      const dlg = await E.waitFor(() => E.modal('That name is already used'), 5000);
      const btn = dlg && [...dlg.querySelectorAll('footer button')].find(b => /Save as/.test(b.textContent));
      const label = btn && btn.textContent;
      btn && btn.click();
      const ok = await saving;
      const list = await __m.api.projects();
      return { asked: !!dlg, label, ok, name: app.store.project.name, listed: list.filter(m => m.uid === Z.uid).map(m => m.file) };
    });
    check('saving under a used name asks first; "Save as" keeps both', ask.asked && ask.ok && /E2E Test Collide \d/.test(ask.name) && ask.listed.length === 1, ask);
  });
  ...
  try {
    const res = await page.evaluate(async (progId, uids) => {
      ...
      const list = await api.projects();
      const removed = [];
      for (const m of list) if (/^E2E Test ([ABC]|Collide)( \d+)?$/.test(m.name) || uids.includes(m.uid)) { await api.removeProject(m.file); removed.push(m.file); }
  ...
  await step(12, 'Library: search, preview, Use this pose, Import as keys', async () => {
    ...
    await pe(() => {
      const m = __e2e.modal('Import as keys'), sel = m && m.querySelector('select');
      const opts = sel ? [...sel.options].map(o => o.value) : [];
      const ok = sel && sel.value === 'smart';
      if (sel) __e2e.setValue(sel, '6');
      return { checks: [['the import dialog starts on "Smart - keys only where needed"', ok, { value: sel && sel.value, opts }]] };
    });
    ...
    await pe(P => {
      ...
      // body keys every 6 frames; the creator's face comes as face keys wherever it changes (face-only between them)
      const bodyKeys = s => s.keys.filter(k => !k.faceOnly);
      C('import makes body keys every 6 frames (+ last frame)', p.sims.every(s => bodyKeys(s).length >= Math.floor((p.length - 1) / 6) + 1 && bodyKeys(s).every(k => k.frame % 6 === 0 || k.frame === p.length - 1)), p.sims.map(s => bodyKeys(s).length));
      C('import keeps the face of the creator (face keys; the ones between body keys are face-only)', p.sims.every(s => s.keys.filter(k => k.faceOnly).every(k => k.faceBones && k.frame % 6 !== 0)) && p.sims.some(s => s.keys.some(k => k.faceBones)),
        p.sims.map(s => ({ faceKeys: s.keys.filter(k => k.faceBones).length, faceOnly: s.keys.filter(k => k.faceOnly).length })));
      const errAt = (f, ref) => {
        const errs = [];
        p.sims.forEach((s, k) => {
          const v = E.view(s);
    ...
    await shot('imported');
    // "Smart" import: keys only where the motion needs them, and it still follows the clip (within 1 degree)
    await pe(async P => {
      const out = [], C = (n, ok, d) => out.push([n, !!ok, d]);
      const imp = await import('/js/dialogs/import.js'), A = __m.animation;
      app.library.startPreview(await __m.api.animation(P.id, 1));
      const pv = app.library.preview;
      if (!pv) return { checks: [['Smart import: a preview is open', false]] };
      const len = Math.min(pv.length - 1, 150);
      imp.importKeys(app, pv, 'smart', 0, len, true);
      const p = app.store.project;
      const body = p.sims.map(s => s.keys.filter(k => !k.faceOnly).length);
      let worst = 0;
      p.sims.forEach((s, k) => {
        const player = pv.players[k];
        for (let f = 0; f < len; f += 3) {
          const want = A.sampleToPose(player.sample(f)), got = A.evaluate(s.keys, f, p.length, p.loop, p.autoCurve);
          for (const n of Object.keys(want.rot)) { const q = want.rot[n], g = got.rot[n]; if (!g) continue; const d = Math.abs(q[0] * g[0] + q[1] * g[1] + q[2] * g[2] + q[3] * g[3]) / (Math.hypot(...q) * Math.hypot(...g)); worst = Math.max(worst, 2 * Math.acos(Math.min(1, d)) * 180 / Math.PI); }
        }
      });
      // (how many keys a clip needs depends on it: smooth hand-keyed clips drop most frames, shaky captured ones fewer)
      C('Smart import keeps fewer keys than frames (only where the motion needs one)', body.every(n => n > 1 && n < len), { body, len });
      C('... and still follows the clip within 1 degree (every 3rd frame, every bone)', worst <= 1.001, { worstDeg: +worst.toFixed(3) });
      return { checks: out };
    }, P);
  });
  ...
  await step(15, 'hover glow, click select, tools, pins, hip turn', async () => {
    ...
    await sleep(200);
    // (the jaw is a face part: picked directly, with the face gizmo and the face card)
    const sel = await page.evaluate(() => { const s = app.store.selected, v = __e2e.view(__e2e.F()); const t = document.getElementById('inspector').textContent; return { sim: s.sim === __e2e.F().id, bone: s.bone, gizmoOn: !!app.vp.gizmo.object && app.vp.gizmo.object === v.bone(s.bone), insp: t.includes('Turn this part') || (/Jaw|Tounge/.test(s.bone || '') && t.includes('Face part')) }; });
    check('clicking the head selects it (or the jaw) with its gizmo', sel.sim && /Head|Neck|Jaw/.test(sel.bone || '') && sel.gizmoOn && sel.insp, sel);
  ...
  try {
    ...
    // no writing request reached the server: every one was answered in the browser (and nothing the test could
    // have written changed in the saves folder: this test's recovery slot, E2E animations)
    check('no writing request reached the server (all answered in the browser)', harness.passed.length === 0, { passed: harness.passed, answeredInBrowser: harness.writes.length });
    const nowSaves = folderState(SAVES, 1);
    const changed = sameState(before.state.saves, nowSaves).filter(k => /E2E|port\d+|test/i.test(k));
    check('the saves folder has nothing from this test', !changed.length, changed);
    info.writesByRoute = harness.writes.reduce((a, w) => { a[w.route] = (a[w.route] || 0) + 1; return a; }, {});
